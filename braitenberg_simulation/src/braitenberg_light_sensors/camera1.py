...
sub = rospy.Subscriber('/camera1/rgb/image_raw', Image, camera1_callback, queue_size = 1,  buff_size=2**24)
pub = rospy.Publisher('/right', Float32, queue_size=5)
rate = rospy.Rate(60)			
cv2.destroyAllWindows()
while not rospy.is_shutdown():
    average_color = (color1 + color2 + color3 + color4 + color5 + color6)
    average_color_value = (average_color[0] + average_color[1] + average_color[2])/3.0
